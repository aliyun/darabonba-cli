...
class Formatter {
  ...
  visitType(ast, level) {
    if (ast.tag === Tag.Tag.TYPE) {
      ...
      this.emit(``);
    } else if (ast.type === 'subModel_or_moduleModel') {
      for (let i = 0; i < ast.path.length; i++) {
        const item = ast.path[i];
        if (i > 0) {
          this.emit('.');
  ...
  visitFor(ast, level) {
    ...
    this.emit(`for (var ${_name(ast.id)} in `, level);
  ...
  visitFieldValue(ast, level) {
    ...
    if (ast.fieldType === 'map') {
      ...
      this.emit(`]`);
      this.visitFieldValue(ast.valueType, level + 1);
      return;
    }

    if (ast.fieldType === 'array') {
      this.emit(`[ `);
      this.visitFieldValue(ast.fieldItemType, level + 1);
      this.emit(` ]`);
      return;
    }
  ...
  visitExpr(ast, level) {
    if (ast.type === 'virtualCall') {
      ...
      } else if (ast.left.type === 'static_or_instance_call') {
        this.emit(`${_name(ast.left.id)}`);
