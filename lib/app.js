...
function color(content) {
  colors.bgRed(colors.white(content));
}
...
class CommandsApplication {
  ...
  run() {
    ...
    console.log();
    console.log(colors.red(`dara: '${commandName}' is not a dara command. See 'dara help'.`));
    console.log();
    process.exit(-1);
  }
  ...
  exec(command) {
    ...
    commandOpts.forEach(opt => {
      if (checkSet.indexOf(opt.name) > -1) {
        console.log();
        console.log(colors.red(`  Duplication option : ${opt.name}`));
        console.log();
        command.usage();
        process.exit(-1);
      }
      ...
      if (opt.short) {
        if (checkSet.indexOf(opt.short) > -1) {
          console.log();
          console.log(colors.red(`  Duplication option short : ${opt.name}(${opt.short})`));
          console.log();
          command.usage();
          process.exit(-1);
        }
    ...
    commandArgs.forEach((arg, key) => {
      if (checkSet.indexOf(arg.name) > -1) {
        console.log();
        console.log(colors.red(`  Duplication argument : ${arg.name}`));
        console.log();
        command.usage();
        process.exit(-1);
      }
      ...
      if (arg.mode === 'required' && arg['value'] === '') {
        console.log();
        console.log(colors.red(`  Required argument : ${arg.name}`));
        console.log();
        command.usage();
        process.exit(-1);
      }
    ...
    commandOpts.forEach(opt => {
      ...
      if (opt.mode === 'required' && opt['value'] === '') {
        console.log();
        console.log(colors.red(`  Required option : ${opt.name}`));
        console.log();
        command.usage();
        process.exit(-1);
      }
    ...
    command.exec(cArgs, cOpts, argv._, this).catch((err) => {
      console.log();
      console.log(colors.red('exec error :'));
