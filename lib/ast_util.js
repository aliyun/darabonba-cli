...
function _upperFirstCase(value) {
...
// format model name from 'model.example.data' to 'modelExampleData'
function _formatModelName(value) {
  if (value.includes('.')) {
    let _values = value.split('.').map((item, index) => {
      if (index) {
        return _upperFirstCase(item);
      }
      return item;
    });
    value = _values.join('');
...
class AstFormatter {
  constructor(pkgDir) {
    ...
    const readmeFilePath = path.join(pkgDir, README_FILE);
  ...
  _parseModel(models) {
    ...
    for (let [modelKey, moduleItem] of Object.entries(models)) {
      modelKey = _formatModelName(modelKey);
  ...
  _formatModelNode(model) {
    ...
    let modelItem = {
      modelName: _formatModelName(model.modelName.lexeme),
      fields: []
    };
    ...
    for (let i = 0; i < nodes.length; i++) {
      ...
      if (item.fieldType === 'array') {
        ...
        if (item.fieldItemType === 'modelBody') {
          item.fieldItemType = modelItem.modelName + _upperFirstCase(item.fieldName);
        }
      }
      ...
      if (item.fieldType === 'modelBody') {
        item.fieldType = modelItem.modelName + _upperFirstCase(item.fieldName);
      }
    ...
    if (model.modelName.lexeme.includes('.')) {
      ...
      let parentModel = _formatModelName(modelArr.join('.'));
